...
def main():
    ...
    ap.add_argument("--outdir", default="results/analysis/global_highest_margin_component_alignment")
    args = ap.parse_args()
    ...
    rows = []
    all_files = sorted(Path().glob(args.pca_glob))
    if not all_files:
        raise SystemExit(f"No PCA files matched: {args.pca_glob}")
    
    # keep only the largest-n PCA file for each config
    best_by_cfg = {}
    for p in all_files:
        meta = _parse_pca_name(p.name)
        if meta is None:
            continue
        key = (meta["dataset"], meta["split"], meta["seed"], meta["layer"], meta["eps"], meta["cc"], meta["tag"], meta["mode"])
        n_val = int(meta["n"])
        if key not in best_by_cfg or n_val > best_by_cfg[key][0]:
            best_by_cfg[key] = (n_val, p)
    
    files = [v[1] for v in best_by_cfg.values()]
    files = sorted(files)
    ...
    for pca_path in files:
        ...
        if args.component_idx >= 0:
            fixed_k = int(args.component_idx)
            reason = "explicit"
            ...
            reason = f"per_dataset_top{n_top}_max_abs_corr_delta_margin"
            top_considered = ",".join(str(x) for x in cand)
        
            label_lookup = label_map.get((dataset, meta["cc"], meta["tag"], meta["mode"]), {})
            top_labels = [label_lookup.get(c, f"comp{c}") for c in cand]
            top_labels_str = " | ".join(f"{c}:{lab}" for c, lab in zip(cand, top_labels))
        ...
        for ds in datasets_in_records:
            ...
            scores_ds_all = scores[idx] if scores is not None else None
            delta_margin_ds = delta_margin[idx]

            if args.component_idx >= 0:
                k = fixed_k
                corr_k_ds = _corr(scores_ds_all[:, k], delta_margin_ds) if scores_ds_all is not None else float("nan")
            else:
                best = None
                for c in cand:
                    r = _corr(scores_ds_all[:, c], delta_margin_ds) if scores_ds_all is not None else float("nan")
            ...
            selected_label = label_map.get((dataset, meta["cc"], meta["tag"], meta["mode"]), {}).get(k, "")
            print(f"[{ds}] candidates: {top_labels_str}")
            print(f"[{ds}] selected: k={k} label={selected_label}")
            ...
            rows.append(
                {
                    ...
                    "top_components_labels": top_labels_str,
